...
function gensysdt(F::GeneralizedSchur, c, Ψ, Π, div)
    ...
    for i in 1:n
        if (abs(a[i, i]) < ϵ) && (abs(b[i, i]) < ϵ)
            @info "Coincident zeros.  Indeterminacy and/or nonexistence." a[i, i] b[i, i]
            eu = [-2, -2]
    ...
    if unique
        ...
        @info "Indeterminacy. $(nloose) loose endogeneous errors" nloose
    end
    ...
    loose = G0I * vcat(etawt1 * (I - (veta * veta')), zeros(nunstab, neta))
